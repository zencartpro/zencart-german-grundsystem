 ...
 * @version $Id: customers_without_order.php 2023-11-03 16:14:51Z webchills $
 */
...
if (!empty($action)) {
    switch ($action) {
        ...
        case 'status':
            if (isset($_POST['current_status']) && ctype_digit($_POST['current_status'])) {
                ...
                zen_redirect(
                    zen_href_link(FILENAME_CUSTOMERS_WITHOUT_ORDER, zen_get_all_get_params(['action']), 'NONSSL')
                );
        }
      $action = '';
      break;
    case 'update':
      ...
      $customers_telephone = zen_db_prepare_input($_POST['customers_telephone']);
            ...
            if ($error === false) {
                ...
                zen_redirect(
                    zen_href_link(
                        FILENAME_CUSTOMERS_WITHOUT_ORDER,
    ...
    case 'deleteconfirm':
            ...
            zen_redirect(zen_href_link(FILENAME_CUSTOMERS_WITHOUT_ORDER, zen_get_all_get_params(['cID', 'action']), 'NONSSL'));
            break;
        default:
            $customer = new Customer($customers_id);
...
?>
        ...
        <div class="row text-right">
            ...
            <a href="<?php
            echo zen_href_link(FILENAME_CUSTOMERS_WITHOUT_ORDER, zen_get_all_get_params(['action'])); ?>"
...
?>
                    <tr>
                        <td colspan="2" class="text-right">
                            <a href="<?php echo zen_href_link(FILENAME_CUSTOMERS_WITHOUT_ORDER); ?>" class="btn btn-default" role="button">

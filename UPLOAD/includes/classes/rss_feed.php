...
class rss_feed extends base {
 ...
 // Die folgenden Zeilen entkommentieren, fals Sie den Download des Artikelbilds als Mediendatei im Feed anbieten wollen
 ...
 //     }
  ...
  function rss_feed_out() {
    if($this->rssFeedCacheFrom == false) {
      $this->rssFeedContent = $this->rss_feed_content();
      $this->rssFeedTimeCreated = time();
      if($this->rssFeedCacheFlag == true) {
        if (($f = fopen($this->rssFeedCacheFileName, 'w'))) {
  ...
  function rss_feed_cache($zf_query, $time=false) {
    ...
    return $this->rssFeedCacheFrom;
  }

  function rssFeedCacheSet($flag) {
    $this->rssFeedCacheFlag = ($flag === true) ? true : false;
  }

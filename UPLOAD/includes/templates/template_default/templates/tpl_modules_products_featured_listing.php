 ...
 * @version $Id: tpl_modules_products_featured_listing.php 730 2016-02-17 18:49:16Z webchills $
 */
?>
...
// BOF Easy Google Analytics module pt1
    ...
    while (!$featured_products->EOF) {
      ...
      if (PRODUCT_FEATURED_LIST_NAME != '0') {
        $display_products_name = '<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($featured_products->fields['master_categories_id']) . '&products_id=' . $featured_products->fields['products_id']) . '"' . $geeOnClick . '><strong>' . $featured_products->fields['products_name'] . '</strong></a>' . str_repeat('<br clear="all" />', substr(PRODUCT_FEATURED_LIST_NAME, 3, 1));
      } else {

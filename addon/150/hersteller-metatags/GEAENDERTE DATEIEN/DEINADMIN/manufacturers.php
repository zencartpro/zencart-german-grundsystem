 ...
 * @version $Id: manufacturers.php for metatags 2012-05-03 09:16:51Z webchills $
 */
...
?>
				...
				<?php if (zen_get_manufacturer_metatags_manu_keywords($manufacturers->fields['manufacturers_id'] , (int)$_SESSION['languages_id']) or zen_get_manufacturer_metatags_manu_description($manufacturers->fields['manufacturers_id'] , (int)$_SESSION['languages_id'])) {
       echo '<a href="' . zen_href_link(FILENAME_MANUFACTURERS, 'page=' . $_GET['page'] . '&mID=' . $manufacturers->fields['manufacturers_id'] . '&action=edit_manufacturer_meta_tags') . '">' . zen_image(DIR_WS_IMAGES . 'icon_edit_metatags_on.gif', ICON_METATAGS_ON) . '</a>'; 
        } else {
        echo '<a href="' . zen_href_link(FILENAME_MANUFACTURERS, 'page=' . $_GET['page'] . '&mID=' . $manufacturers->fields['manufacturers_id'] . '&action=edit_manufacturer_meta_tags') . '">' . zen_image(DIR_WS_IMAGES . 'icon_edit_metatags_off.gif', ICON_METATAGS_OFF) . '</a>'; 
		}?>
...
<?php
  ...
  switch ($action) {
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
      ...
      $manufacturer_inputs_string_metatags_keywords .= zen_draw_textarea_field('metatags_keywords[' . $languages[$i]['id'] . ']', 'soft', '100%', '20', htmlspecialchars(zen_get_manufacturer_metatags_manu_keywords($manufacturers_id, $languages[$i]['id']), ENT_COMPAT, CHARSET, TRUE));
    }
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
      $manufacturer_inputs_string_metatags_description .= '<br />' . zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['metatags_description']) . '&nbsp;' ;
      $manufacturer_inputs_string_metatags_description .= zen_draw_textarea_field('metatags_description[' . $languages[$i]['id'] . ']', 'soft', '100%', '20', htmlspecialchars(zen_get_manufacturer_metatags_manu_description($manufacturers_id, $languages[$i]['id']), ENT_COMPAT, CHARSET, TRUE));
    }
    $contents[] = array('text' => '<br />' . TEXT_EDIT_MANUFACTURER_META_TAGS_DESCRIPTION . $manufacturer_inputs_string_metatags_description);

		...
		*/
		...
		if(($_POST['add_edit'] == 'add') || ($_GET['add_edit'] == 'add')) //s_mack:noconfirm
		{
			if (preg_match("/\|/", $attributes)) {
				...
				// now loop through each list of variables / attributes
				// and add each one to the database
				for ($i = 0;$i < sizeof($arrNew);$i++) {
					$strAttributes = implode(",", $arrNew[$i]);
// 					echo "attr: $strAttributes<bR>\n";
					
					// make sure there is an on duplicate key so that keys get updated if
					// the stock / attribute information is already present

 ...
 */
  ...
  define('COMPUTE_TAX_ON_POST_DISCOUNT_PRICE', 'true');  
  class ot_coupon {
    var $title, $output;
  ...
  function process() {
    ...
    if ($od_amount['total'] > 0) {
      ...
      while (list($key, $value) = each($order->info['tax_groups'])) {
        ...
        if ($od_amount[$key]) {
           if (DISPLAY_PRICE_WITH_TAX != 'true') {
             ...
             if (COMPUTE_TAX_ON_POST_DISCOUNT_PRICE == 'true') { 
                 $order->info['tax_groups'][$key] -= ($od_amount['total'] - $od_amount['total']/(1+($tax_rate/100.0)));
             }
           }
      ...
      if (DISPLAY_PRICE_WITH_TAX == 'true') {
        ...
        $reduce_amt = $od_amount['total'] - $tax;
      } else {
        $reduce_amt = $od_amount['total'];
      }
      $order->info['total'] = $order->info['total'] - $reduce_amt;

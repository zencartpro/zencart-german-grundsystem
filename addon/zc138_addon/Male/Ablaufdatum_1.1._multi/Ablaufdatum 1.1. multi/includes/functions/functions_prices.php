...
$enddatum = '';
	...
	if ($abvkorsp->fields['sale_specials_condition'] == '0') {
		if ($angebotsende2->fields['sale_date_end'] != '0001-01-01') {
			$enddatum = SPECIAL_PRICE_VALID_UNTIL . strftime(DATE_FORMAT_SHORT, strtotime($angebotsende2->fields['sale_date_end'])-86400); 
		}
	} elseif ($abvkorsp->fields['sale_specials_condition'] == '1') {
		if ($istsp->fields['status'] == '1') {
			if ($angebotsende->fields['expires_date'] != '0001-01-01') {
				$enddatum = SPECIAL_PRICE_VALID_UNTIL . strftime(DATE_FORMAT_SHORT, strtotime($angebotsende->fields['expires_date'])-86400); 
			}
		} else {
			if ($angebotsende2->fields['sale_date_end'] != '0001-01-01') {
				$enddatum = SPECIAL_PRICE_VALID_UNTIL . strftime(DATE_FORMAT_SHORT, strtotime($angebotsende2->fields['sale_date_end'])-86400); 
...
// show case only
    ...
    if ($display_special_price) {
      ...
      if ($display_sale_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          $show_normal_price = '<s>' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>' . TAX_ADDON;
        } else {
          $show_normal_price = $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . TAX_ADDON;
        }
        $show_special_price = '';
        $show_sale_price = '';
      }

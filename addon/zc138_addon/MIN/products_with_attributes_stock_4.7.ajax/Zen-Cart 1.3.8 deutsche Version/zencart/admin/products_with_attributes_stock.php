 ...
 * @version $Id: products_with_attributes_stock.php 515 2009-02-26 16:54:18Z webchills $
 */
...
switch($action)
{
	...
	case 'execute':
		...
		*/
		...
		$messageStack->add_session(PWA_MESSAGE_PRODUCT_QUANTITY_UPDATED_SUCCESS, 'success');
	...
	case 'delete':
		...
		else
		{
			...
			if($_POST['confirm'] == TEXT_YES){
				...
				$messageStack->add_session(PWA_MESSAGE_DELETE_VARIANT_SUCCESS, 'failure');
				zen_redirect(zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, '', 'NONSSL'));
			} else {
				zen_redirect(zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, '', 'NONSSL'));
			}
	...
	case 'resync':
		if(is_numeric((int)$_GET['products_id'])){
			...
			$messageStack->add_session(PWA_MESSAGE_PARENT_QUANTITY_UPDATED_SUCCESS, 'success');
			zen_redirect(zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, '', 'NONSSL'));

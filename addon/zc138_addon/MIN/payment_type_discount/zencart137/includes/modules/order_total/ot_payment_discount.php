...
class ot_payment_discount
{
  ...
  {
  global $db;
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Modul aktivieren?', 'MODULE_PAYMENT_DISCOUNT_STATUS', 'true', 'Wollen Sie den Rabatt f&uuml;r die Zahlungsart aktivieren?', '6', '1','zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Anzeigereihenfolge', 'MODULE_PAYMENT_DISCOUNT_SORT_ORDER', '220', 'Anzeigereihenfolge. Voreingestellt ist 220. Falls das ge&auml;ndert wird, darauf achten, dass dieses Modul vor der Steuerberechnung kommt.', '6', '2', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Rabatt Prozentsatz', 'MODULE_PAYMENT_DISCOUNT_DISCOUNT_PERCENTAGE', '3.00', 'H&ouml;he des Rabatts in Prozent:', '6', '7', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Mindestbestellwert f&uuml;r den Rabatt', 'MODULE_PAYMENT_DISCOUNT_DISCOUNT_MINIMUM', '100', 'Welchen Mindestwert muss die Bestellsumme haben, damit der Rabatt gew&auml;hrt wird? Voreingestellt: 100', '6', '2', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zahlungsweisen, die den Rabatt erhalten', 'MODULE_PAYMENT_DISCOUNT_DISCOUNT_TYPES', 'moneyorder', 'Bei welchen Zahlungsweisen wollen Sie den Rabatt gew&auml;hren?<br />Voreingestellt: moneyorder. Sie k&ouml;nnen mehrere Zahlungsweisen angeben, sie m&uuml;ssen mit Komma getrennt sein.<br/>Als Name MUSS die jeweilige Klasse angegeben werden, z.B. cod,moneyorder,cc', '6', '2', now())");
     }

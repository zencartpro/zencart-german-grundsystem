...
/*
        payguard.de
        FMCE Commerce Process Platform
        version 1.5.01  2010/06/07
        ...
        New methods 2010: Frank Maroke <info@fmce.de>
        modified for Zen-Cart 1.3.8 german 2010-10-09 webchills
*/
...
// Auffüllen der Konto-Nr. mit '0' auf 10 Stellen
  ...
  // Erweiterte cpp_ExpandAccount fuer Methode C5:
 ...
 function Mark09($AccountNo) {
  ...
  function Mark27($AccountNo) {
    if ((int)$AccountNo <= 999999999.) {
      $cpp_Mark27 = $this->cpp_Method00($AccountNo, '212121212', 10);
    } else {
  ...
  function MarkB6($AccountNo, $BLZ) {
    ...
    if (intval(substr($AccountNo,0 ,1)) <= 9 && intval(substr($AccountNo,0 ,1)) > 0 ){
      ...
      if (strlen($AccountNoShort) == 9) {
        $RetVal = $this->cpp_Mark53($AccountNoShort, $BLZ);
      } else {
        $RetVal = 1;
      }
    }
 ...
 function MarkC0($AccountNo, $BLZ) {
  ...
  function MarkC6($AccountNo) {
    ...
    // 0, 1, 2, 3, 7, 9 an erster Stelle von links = mit Methode 00 pruefen.
      ...
      // Pruefziffer NICHT verschieben, cpp_ExpandAccount NICHT anwenden.
  ...
  function MarkD2($AccountNo) {
    ...
    $markD2 = $this->cpp_Mark95($AccountNo);

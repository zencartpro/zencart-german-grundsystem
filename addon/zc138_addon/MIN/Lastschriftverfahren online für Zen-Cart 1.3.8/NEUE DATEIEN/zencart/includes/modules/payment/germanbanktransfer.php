 ...
 * @version $Id: germanbanktransfer.php 157 2005-04-07 20:33:35Z dogu $
 * @version $Id: germanbanktransfer.php multilanguage for ZC138 german 667 2010-10-09 webchills $
 */
...
//      zen_session_unregister('gbt_number');
    ...
    function install() {
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order:', 'MODULE_PAYMENT_GERMANBT_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added)values ('Get Bank codes from database?', 'MODULE_PAYMENT_GERMANBT_DATABASE_BLZ', 'true', 'Should we search in the database for the bank codes? It is strongly recommended to set this to true! Make sure that you have imported the bank codes before into the table banktransfer_blz!', '6', '0', 'zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Lastschrift ab x-ter Bestellung möglich?', 'MODULE_PAYMENT_GERMANBT_ENABLE_AFTER', '43', 'Möchten Sie die Zahlung per Lastschrift nach der X-ten Bestellung erlauben?', now())");

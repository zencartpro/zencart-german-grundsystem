 ...
 * @version $Id: functions_addresses.php 2024-03-28 15:36:16Z webchills $
 */
...
/**
 * Returns an array with countries for multilanguage country names
 *
...
function zen_get_countries(int $country_id = 0, bool $with_iso_codes = false, bool $activeOnly = true)
{
    ...
    if (!empty($country_id)) {
        $sql .= " WHERE co.countries_id = " . (int)$country_id;
        if ($activeOnly) $sql .= " AND co.status != 0 ";
    } else {

 ...
 * @version $Id: curltester.php 2024-03-15 19:23:29Z webchills $
 */
// no caching
...
?>
    ...
    <head>
        ...
        <title>Communications Test</title></head>
<body>
<p>
<?php
...
<em>For advanced "details" mode, add </em><strong>?details=on</strong><em> to the URL.</em></p>
    <p><em>(Another resource you may find useful for testing your server's overall customer-facing SSL configuration:
            <a href="https://www.ssllabs.com/ssltest/index.html" rel="noreferrer noopener" target="_blank">https://www.ssllabs.com/ssltest/index.html</a>
            )
        </em>
    </p>
...
function doCurlTest($url = 'http://s3.amazonaws.com/zencart-curltest/endpoint', $postdata = "field1=This is a test&statuskey=ready")
{
    ...
    curl_setopt($ch, CURLOPT_USERAGENT, 'Zen Cart(tm) - CURL TEST v200');
...
//  curl_setopt($ch, CURLOPT_CAINFO, '/local/path/to/cacert.pem');
    ...
    if (in_array($errnum, [35])) {
        ...
        // Using the defined value of 6 instead of CURL_SSLVERSION_TLSv1_2 in case the outdated host is also using older PHP versions which don't know of this constant
    ...
    if (in_array($errnum, [60, 61])) {
        ...
        // This is intentional, only in this scenario of inspecting causes of certificate failures.
...
function dofsockTest($url = 's3.amazonaws.com/zencart-curltest/endpoint', $port = 80, $timeout = 5)
{
    ...
    if ($socket) {
        ...
        echo '<br>' . $errorMessage . ' Num: ' . $errnum . ', Message: ' . $errtext . '<br><br>';
    }
    if ($showDetails) {
        echo '<hr>';
    }

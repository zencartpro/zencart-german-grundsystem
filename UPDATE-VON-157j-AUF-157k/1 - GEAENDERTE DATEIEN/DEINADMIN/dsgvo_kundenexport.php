 ...
 * @version $Id: dsgvo_kundenxport.php 2026-04-04 18:24:51Z webchills $
 */
...
$dsgvo_export_array = $db->Execute("SELECT 
    ...
    fputcsv($output, array(DSGVO_CUSTOMERDATA_HEADING),';',escape: "");
    fputcsv($output, array(''),';',escape: "");
    ...
    foreach ($dsgvo_export_array->fields as $key=>$value) {
        ...
        fputcsv($output, array(constant('DSGVO_'.strtoupper($key)),$value),';',escape: "");           
    }     
    fputcsv($output, array(''),';',escape: "");
    ...
    $dsgvo_orders_array = $db->Execute("SELECT *
        ...
        fputcsv($output, array(DSGVO_ORDERS_HEADING),';',escape: "");
        fputcsv($output, array(''),';',escape: "");  
...
while (!$dsgvo_orders_array->EOF) {        
            ...
            fputcsv($output, array(DSGVO_ORDER_HEADING." ".$oNum),';',escape: ""); 
            fputcsv($output, array(DSGVO_ORDER_ID,$dsgvo_orders_array->fields['orders_id']),';',escape: ""); 
            fputcsv($output, array(DSGVO_ORDER_DATE,$dsgvo_orders_array->fields['date_purchased']),';',escape: ""); 
            fputcsv($output, array(DSGVO_ORDER_IP_ADDRESS,$dsgvo_orders_array->fields['ip_address']),';',escape: ""); 
                            
            fputcsv($output, array(DSGVO_CUSTOMER_ADDRESS,$dsgvo_orders_array->fields['customers_name'],$dsgvo_orders_array->fields['customers_street_address'],$dsgvo_orders_array->fields['customers_postcode'].' '.$dsgvo_orders_array->fields['customers_city'],$dsgvo_orders_array->fields['customers_country']),';',escape: ""); 
            fputcsv($output, array(DSGVO_SHIPPING_ADDRESS,$dsgvo_orders_array->fields['delivery_name'],$dsgvo_orders_array->fields['delivery_street_address'],$dsgvo_orders_array->fields['delivery_postcode'].' '.$dsgvo_orders_array->fields['delivery_city'],$dsgvo_orders_array->fields['delivery_country']),';',escape: ""); 
            fputcsv($output, array(DSGVO_BILLING_ADDRESS,$dsgvo_orders_array->fields['billing_name'],$dsgvo_orders_array->fields['billing_street_address'],$dsgvo_orders_array->fields['billing_postcode'].' '.$dsgvo_orders_array->fields['billing_city'],$dsgvo_orders_array->fields['billing_country']),';',escape: ""); 
            
            fputcsv($output, array(DSGVO_PAYMENT_METHOD,$dsgvo_orders_array->fields['payment_method']),';',escape: ""); 
...
while (!$attribute_query->EOF) {
                    ...
                    if ($first_product) {
                        fputcsv($output, array(DSGVO_ORDER_HEADING,$dsgvo_product_array->fields['products_quantity']." x ".$dsgvo_product_array->fields['products_name']." ".$attr_string,DSGVO_PRODUCT_NUMBER.": ".$dsgvo_product_array->fields['products_model']),';',escape: "");  
                        $first_product = false;
                    } else {
                        fputcsv($output, array('',$dsgvo_product_array->fields['products_quantity']." x ".$dsgvo_product_array->fields['products_name']." ".$attr_string,DSGVO_PRODUCT_NUMBER.": ".$dsgvo_product_array->fields['products_model']),';',escape: ""); 
                    }  
            ...
            while (!$dsgvo_order_total_array->EOF) {
                fputcsv($output, array('','','', strip_tags(html_entity_decode($dsgvo_order_total_array->fields['title'])).strip_tags($dsgvo_order_total_array->fields['text'])),';',escape: "");

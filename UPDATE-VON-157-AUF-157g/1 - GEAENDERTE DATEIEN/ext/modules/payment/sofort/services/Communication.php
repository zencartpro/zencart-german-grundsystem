<?php
// fixes for 1.5.7g and PHP 8.1.x - 2023-12-20 - webchills
    ...
    public function handleSofortStatusUpdate(array $statusData)
    {
        ...
        if (array_key_exists($statusData['status'], $allowedStates)
            ...
            if (!empty($payment->$property)) {
                ...
                $query1 = $db->Execute("SELECT * FROM `". DB_PREFIX . "pi_sofort_transaction` WHERE transaction_id = '" . $statusData['transaction_id'] . "' LIMIT 1");
                
                if (!$query1->EOF) {
                $query2 = $db->Execute("SELECT * FROM `" . TABLE_ORDERS . "` WHERE orders_id = " . (int) $query1->fields['order_id']);               

                    if ((!$query2->EOF) && ($query2->fields['orders_status'] !== $payment->$property)) {

                        $sql_data_array = array(
                            'orders_id' => $query2->fields['orders_id'],
                        ...
                        if ($check_query->fields['orders_status_id'] !== $payment->$property) {
                            $db->Execute("UPDATE `" . TABLE_ORDERS . "` SET orders_status = " . (int) $payment->$property . " WHERE orders_id = " . (int) $query2->fields['orders_id']);

...
if (!defined('TEXT_RESELECT_SHIPPING')) {
    define('TEXT_RESELECT_SHIPPING', 'You have changed the items in your cart since shipping was last calculated, and costs may have changed. Please verify/re-select your shipping method.');
}
...
//    zen_redirect(zen_href_link(FILENAME_TIME_OUT));
  ...
  if (isset($_GET['clearSess']) || isset($_GET['amp;clearSess']) || isset($_GET['ec_cancel']) || isset($_GET['amp;ec_cancel'])) {
    // Unset the PayPal EC information.
        unset($_SESSION['paypal_ec_temp'], $_SESSION['paypal_ec_token'], $_SESSION['paypal_ec_payer_id'], $_SESSION['paypal_ec_payer_info']);
    }
    ...
    if (defined('MODULE_PAYMENT_PAYPALWPP_STATUS') && MODULE_PAYMENT_PAYPALWPP_STATUS === 'True') {
        ...
        // set the payment, if they're hitting us here then we know the payment method selected right now.
        $_SESSION['payment'] = $paypalwpp_module;
        // check to see if we have a token sent back from PayPal.
        if (empty($_SESSION['paypal_ec_token'])) {
            // We have not gone to PayPal's website yet in order to grab a token at this time.
            // This will send the customer over to PayPal's website to login and return a token
            ${$paypalwpp_module}->ec_step1();
        } else {
            // This will push on the second step of the paypal EC payment module,
            // as we already have a PayPal express checkout token at this point.
...
Processing...
   ...
   */
    if (isset($_POST['test_ipn']) && (int)$_POST['test_ipn'] === 1) {
        ipn_debug_email('IPN NOTICE :: Processing SANDBOX transaction.');
    }
    if (isset($_POST['test_internal']) && (int)$_POST['test_internal'] === 1) {
  ...
  switch ($txn_type) {
    ...
    case 'unique':
      ...
      if ($ipnFoundSession === false && !$isECtransaction && !$isDPtransaction) {
        ...
        $_SESSION['order_summary']['orders_status'] = $order->info['orders_status'];
                ...
                if (!($new_record_needed && $txn_type === 'echeck-cleared')) {
                    break;
                }
      }
    case 'parent':
    ...
    case 'express-checkout-cleared':
      ipn_debug_email('IPN NOTICE :: Storing order/update details for order #' . $ordersID . ' txn_id: ' . $_POST['txn_id'] . ' PP IPN ID: ' . $paypalipnID);
            ...
            zen_db_perform(TABLE_PAYPAL_PAYMENT_STATUS_HISTORY, $sql_data_array);
            ipn_debug_email(
                'IPN NOTICE :: Added PP status-history record for order #' . $ordersID . ' txn_id: ' . $_POST['txn_id'] . ' (updated/child) PP IPN ID: ' . $paypalipnID
            );
    ...
    switch (true) {
        ...
        case ($txn_type === 'pending-multicurrency' && (int)$ordersID > 0):
            ipn_debug_email(
                'IPN NOTICE :: Pending multicurrency transaction for existing order. No action required. Waiting for merchant to "accept" the order via PayPal account console.'
            );
            break;
        case ($txn_type === 'pending-address' && (int)$ordersID > 0):
            ipn_debug_email(
                'IPN NOTICE :: "Pending address" transaction for existing order. No action required. Waiting for address approval by store owner via PayPal account console.'
            );
            break;
        case ($txn_type === 'pending-paymentreview' && (int)$ordersID > 0):
            ipn_debug_email(
                'IPN NOTICE :: "Pending payment review" transaction for existing order. No action required. Waiting for PayPal to complete their Payment Review. Do not ship order until review is completed.'
            );
            break;
    }
}

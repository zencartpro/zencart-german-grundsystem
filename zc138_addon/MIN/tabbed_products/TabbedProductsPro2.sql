...
### for multiligual installations
DELETE FROM configuration_language WHERE configuration_group_id = @gid;
...
INSERT INTO configuration_language (configuration_title, configuration_key, configuration_description, configuration_group_id)
    VALUES 
    ('TPP - Global Enable Tabs', 'GLOBAL_ENABLE_TABS', 'Set this to 1 if you want to enable the global use of tabs on your products', @gid),
    ('TPP - Global Main Image On Tab', 'GLOBAL_MAIN_IMAGE_ON_TAB', 'Set this to 1 if you want all products to have the Main Image on the first (Product Description) tab', @gid),
    ('TPP - Global Product Description Tab', 'GLOBAL_PROD_DESC_ON_TAB', 'Set this to 1 if you want all products to have the Main Product Description on its own tab', @gid),
    ('TPP - Global Attributes on Tab', 'GLOBAL_ATTRIBUTES_ON_TAB', 'Set this to 1 if you want the Attributes to appear on their own tab. This will only show up if the product has attributes to show. If there are no attributes, the tab will not show up.', @gid),
    ('TPP - Global Attributes on Add-to-Cart Tab', 'GLOBAL_ATTRIBUTES_ON_ATC_TAB', 'Set this to 1 if you want the Attributes to appear on the Add-To-Cart tab. They will only show up if the product has attributes to show. YOU MUST ALSO LEAVE THE Global Attributes on Tab OPTION ENABLED. This will override the standalone tab and show the attributes on the Add-To-Cart tab only. If the Add-To-Cart tab is set to false, this will be ignored and the attributes will follow their own tab settings.', @gid),
    ('TPP - Global Details On Tabs', 'GLOBAL_DETAILS_ON_TAB', 'Set this to 1 if you want the Product Details to appear on their own tab (weight, model number, etc). This will only show up if the product has details enabled. If there are no product details, the tab will not show up. ', @gid),
    ('TPP - Global Add To Cart Tab', 'GLOBAL_ADD_TO_CART_ON_TAB', 'Set this to 1 if you want the add to cart button to be on its own tab. Note this includes Add To Cart button and Qty Discounts table.', @gid),
    ('TPP - Global Additional Images Tab', 'GLOBAL_ADDL_IMAGES_ON_TAB', 'Set this to 1 if you want the additional images for a product on its own tab. This will only show up if the product has additional images to show. If there are no additional images, the tab will not show up.', @gid),
    ('TPP - Global Customers Also Purchased Tab', 'GLOBAL_CUST_ALSO_PURCH_ON_TAB', 'Set this to 1 if you want the Customers Also Purchased module on its own tab. This will only show up if the product has other products to show. If there are no other products, the tab will not show up.', @gid),
    ('TPP - Global Cross Sell Tab', 'GLOBAL_CROSS_SELL_ON_TAB', 'Set this to 1 if you want the Cross Sell module on its own tab. This will only work if Cross Sell contrib is already installed and the product has cross sell items set up. Otherwise it will just not show up.', @gid),
    ('TPP - Global Reviews Tab', 'GLOBAL_REVIEWS_ON_TAB', 'Set this to 1 if you want the Product Reviews to show up on its own tab. This will always show up even if there are no reviews. The Review module has its own "No reviews found" so it defaults.', @gid),
    ('TPP - Show Tab Headers', 'SHOW_TAB_HEADERS', 'Set this to 1 if you want a header bar to appear under the tabs, above the tab content', @gid);                

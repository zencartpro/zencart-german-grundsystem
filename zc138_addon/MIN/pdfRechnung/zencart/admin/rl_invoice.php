...
generates pdf-invoices; please read: readme_rl_invoice.pdf|txt 
 ...
 $ modified with Euro Patch and utf-8 compability for use in 1.3.8 2008/09/08 20:05:12 webchills $
 */
...
class rlInvoice{
     ...
     function rlInvoice($oID){
         ...
         $this -> logoFile = '../' . RL_INVOICE_LOGO_SHOP_IMAGE; 
         $this -> addressAfter = $this -> getDefault(RL_INVOICE_ADDRESS_AFTER, array('y' => '216', 'deltaY' => '30'));
         $this -> logoRow1 = $this -> getDefault(RL_INVOICE_ROW1, array('text' => 'Row1', 'fontSize' => '9', 'x' => '0', 'y' => '0'));
         $this -> logoRow2 = $this -> getDefault(RL_INVOICE_ROW2, array('text' => 'Row2', 'fontSize' => '9', 'x' => '0', 'y' => '0'));
         $this -> logoRow3 = $this -> getDefault(RL_INVOICE_ROW3, array('text' => 'Row3', 'fontSize' => '9', 'x' => '0', 'y' => '0'));
     ...
     function createPDF(){
         ...
         */
         ...
         $x[0] = iconv("UTF-8","ISO-8859-1",$x[0]);  
         $aml = $realPW / 2;
         $pdf -> ezText($x[0], 12, array('left' => $aml));
        /**
         * date, InvoiceNo, ..
         */
         ...
         */
         ...
         foreach($this -> order -> products as $key => $val){
             ...
             $data[$i]['singleE'] = html_entity_decode($this -> currencies -> format($val['final_price'], true, $this -> order -> info['currency'], $this -> order -> info['currency_value'])) . "  ";
             ...
             $data[$i]['sumE'] = html_entity_decode($this -> currencies -> format($val['qty'] * ($val['final_price']) + $val['onetime_charges'], true, $this -> order -> info['currency'], $this -> order -> info['currency_value'])) . "  ";
             
              $data[$i]['sumE'] = EuroPatch($data[$i]['sumE']); // Patch Zeile
             
             $data[$i]['sumI'] = html_entity_decode($this -> currencies -> format($val['qty'] * ($val['final_price'] + $val['final_price'] * $val['tax'] / 100) + ($val['onetime_charges'] + $val['tax'] * $val['onetime_charges'] / 100), true, $this -> order -> info['currency'], $this -> order -> info['currency_value'])) . "  ";
         ...
         */
         ...
         $pdf -> ezText($this -> footer['text'], $this -> footer['fontsize']);
        /**

     ...
     function createPDF(){
         ...
         */
         ...
         foreach($this -> order -> products as $key => $val){
             ...
             $data[$i]['singleE'] = html_entity_decode($this -> currencies -> format($val['price'], true, $this -> order -> info['currency'], $this -> order -> info['currency_value']));
             $data[$i]['singleI'] = html_entity_decode($this -> currencies -> format($val['price'] + $val['price'] * $val['tax'] / 100, true, $this -> order -> info['currency'], $this -> order -> info['currency_value']));
             ...
             $data[$i]['sumE'] = html_entity_decode($this -> currencies -> format($val['qty'] * ($val['price']) + $val['onetime_charges'], true, $this -> order -> info['currency'], $this -> order -> info['currency_value']));
             $data[$i]['sumI'] = html_entity_decode($this -> currencies -> format($val['qty'] * ($val['price'] + $val['price'] * $val['tax'] / 100) + ($val['onetime_charges'] + $val['tax'] * $val['onetime_charges'] / 100), true, $this -> order -> info['currency'], $this -> order -> info['currency_value']));
             $i++;
             }
         $pdf -> ezSetDy(-15);
         if($_GET['test'] == 'PDF'){

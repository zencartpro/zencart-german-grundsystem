...
//      zen_session_unregister('gbt_number');
    ...
    function install() {
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added)values ('Datenbankbasierte BLZ Abfrage?', 'MODULE_PAYMENT_GERMANBT_DATABASE_BLZ', 'false', 'Sollen die Bankleitzahlen in der Datenbank gesucht werden? Stellen Sie sicher, dass die Tabelle banktransfer_blz existiert und mit Inhalt befüllt wurde!', '6', '0', 'zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Fax-URL','MODULE_PAYMENT_GERMANBT_URL_NOTE', 'fax.html', 'The fax-confirmation file. It must located in catalog-dir', '6', '0', now())");

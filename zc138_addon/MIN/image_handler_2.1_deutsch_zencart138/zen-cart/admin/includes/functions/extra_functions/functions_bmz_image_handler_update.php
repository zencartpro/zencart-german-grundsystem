 ...
 * @modified for multilanguage use in Zen-Cart 1.3.8 german Copyright 2008 webchills.at
 * @license http://www.gnu.org/licenses/gpl.txt GNU General Public License V2.0
 * @version $Id: functions_bmz_image_handler_update.php,v 1.9 2008-12-13 12:53:00 webchills $
 */
...
function install_image_handler() {
  ...
  //--------------------------------------
  // SMALL_IMAGE_HOTZONE configuration and configuration language entry
  //--------------------------------------
  ...
  $db->Execute($sql_query);
  $sql_query = "INSERT INTO " . TABLE_CONFIGURATION_LANGUAGE . " (configuration_title, configuration_key, configuration_language_id, configuration_description, last_modified, date_added) VALUES " .  
				"('IH - Kleine Bilder - Hotzone', 'SMALL_IMAGE_HOTZONE', '43', 'Stellen Sie auf ''yes'', wenn im kleinen Bildern bei Mouseover eine Hotzone angezeigt werden soll. Der Zoom erfolgt dann nur beim Mouseover über die Hotzone und nicht über das ganze Bild.', now(), now());";
	$db->Execute($sql_query);
	...
	//---------------------------------------
	// ZOOM_MEDIUM_IMAGES configuration and configuration language entry
	//---------------------------------------
	...
	$db->Execute($sql_query);
	$sql_query = "INSERT INTO " . TABLE_CONFIGURATION_LANGUAGE . " (configuration_title, configuration_key, configuration_language_id, configuration_description, last_modified, date_added) VALUES " .  
				"('IH - Mittlere Bilder - Zoom', 'ZOOM_MEDIUM_IMAGES', '43', 'Stellen Sie auf ''yes'', wenn Sie beim Mouseover über mittlere Bilder einen Zoomeffekt anzeigen lassen wollen.', now(), now());";
	$db->Execute($sql_query);

  //-----------------------------------------
  // MEDIUM_IMAGE_HOTZONE configuration and configuration language entry
  //-----------------------------------------
  ...
  $db->Execute($sql_query);
  $sql_query = "INSERT INTO " . TABLE_CONFIGURATION_LANGUAGE . " (configuration_title, configuration_key, configuration_language_id, configuration_description, last_modified, date_added) VALUES " .  
				"('IH - Mittlere Bilder - Hotzone', 'MEDIUM_IMAGE_HOTZONE', '43', 'Stellen Sie auf ''yes'', wenn im kleinen Bildern bei Mouseover eine Hotzone angezeigt werden soll. Der Zoom erfolgt dann nur beim Mouseover über die Hotzone und nicht über das ganze Bild.', now(), now());";
	$db->Execute($sql_query);
	...
	//---------------------------------
	// ZOOM_GRAVITY configuration and configuration language entry
	//---------------------------------
	$sql_query = '';
	if (!defined('ZOOM_GRAVITY')) {
		...
		$db->Execute($sql_query);
		$sql_query = "INSERT INTO " . TABLE_CONFIGURATION_LANGUAGE . " (configuration_title, configuration_key, configuration_language_id, configuration_description, last_modified, date_added) VALUES " .  
				"('IH - Zoom - Position', 'ZOOM_GRAVITY', '43', 'Wählen Sie die Position der Anzeige für den Zoom. Voreingestellt ist <strong>SouthEast (SüdOst)</strong>.', now(), now());";
	$db->Execute($sql_query);
...
function update_image_handler() {
  ...
  // 2.1 UPDATE
  $version = '2.1 for Zen-Cart 1.3.8 german';
